...
export default {
  ...
  methods: {
    ...
    get_block(message) {
      ...
      axios
        .get(
          "http://www.yutiansut.com:8010/stock/block?block=" + this.block_ref
        )
        .then(response => {
    ...
    select(id) {
      this.monitorCode.push({
        block: this.block_ref,
        code: this.block_code[id]["code"],
        name: this.block_code[id]["name"]
      });
    },
    save() {
      console.log({ block: JSON.stringify(this.monitorCode) });
      axios.post(
        "http://www.yutiansut.com:8010/user/blocksetting?block=" +
          JSON.stringify(this.monitorCode)
      );
    },
    get() {
      axios
        .get("http://www.yutiansut.com:8010/user/blocksetting")
        .then(response => {
